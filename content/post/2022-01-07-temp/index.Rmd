...
根据面积比可得$\hat\pi=4\frac{^{ }S_c}{S_s}$。当投掷5000个点的时候,$\hat\pi=\frac{3925\times4}{5000}=$ `r 4*sum(df01[3]/N)`。以实验投掷散点数量为X轴，以估的$\hat\pi$为Y轴绘图如下。很明显地可以看出随着点的数量增多$\hat\pi$慢慢向真实的值（蓝线）逼近。
