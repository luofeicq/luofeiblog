...
mod_all <- lm( case_all ~ days + I(days^2), data = mod_data)
...
mod_data %>% ggplot() +
  ...
  geom_text(data = mod_data %>% filter(rep_date > as.Date("2022-4-7")), aes(rep_date, case_all, label = case_all), col = "red4", nudge_x = - 1, size =3) +
  ...
  geom_text(data = pre_data %>% filter(Date >  as.Date("2022-4-7")) , 
            aes(Date, pred_all, label =  pred_all),col = "green4", nudge_x= 0, nudge_y = -500, size = 3) +  
  theme_bw() +
...
```{r}
page_loca <- read_html("https://mp.weixin.qq.com/s/HTM47mUp0GF-tWXkPeZJlg")
page_old <- read_html("https://wsjkw.sh.gov.cn//xwfb/20220324/a8643ae53c7644f597cd88b8f2f4d8c4.html")

page_old %>% html_nodes("p") %>% html_text() %>% .[str_detect(., "居住于")]
```
