...
mod_data <-  tb_fenlei_final %>% select(rep_date, case_all,case_c, case_u) %>% arrange(rep_date) %>% 
  mutate(days = 1:nrow(tb_fenlei_final))
mod_all <- lm( case_all ~ days + I(days^2 + I(days^3)),data = mod_data)
mod_c <- lm( case_c ~ days + I(days^2 + I(days^3)),data = mod_data)
mod_u <- lm( case_u ~ days + I(days^2 + I(days^3)),data = mod_data)
newdata <- data.frame(days =  1  : c(nrow(mod_data) + 3), Date = seq( as.Date("2022-3-16"), by = "day", length.ou = 25  )  )


pre_data <- newdata %>% mutate(
  pred_all = predict(mod_all, newdata) %>% round(0),
  pred_c = predict(mod_c, newdata) %>% round(0),
  pred_u = predict(mod_u, newdata) %>% round(0)
)
...
mod_data %>% ggplot() +
  ...
  geom_text(data = pre_data %>% filter(Date >  as.Date("2022-4-6")) , 
            aes(Date, pred_all, label =  pred_all), nudge_x= 1, nudge_y = -1000, size = 3) +  
  theme_bw() +
...
gam(data = mod_data, case_all ~ days) %>% summary()
 gamSim(1,n=400,dist="normal",scale=2)  
```
...
initialstat <- c(S = 0.999, #Proportion fo Susceptible population
                 E = 0, # Incubation pop
                 I = 0.001, #Infection pop
                 R = 0 )# recovery pop.

params <- c(b = 0.56, # The possibility of  the susceptible became infected within being  attached  by one infected case.
            c = 0.23, # The rate of the E became I
            r = 0.07 # The recovery rate of the infection pop.
            )

times <- c(1:22) #The period for the model. 
...
plot(out)
#Advanced plot
rc <- round(params[1]/params[3] ,2)
mydf <- gather(data.frame(out), key = "PopTypes", value = "Cases", S:R)
fig1 <- ggplot(mydf, aes(x = time, y = Cases)) + geom_line(aes(col = PopTypes), size = 1) +
