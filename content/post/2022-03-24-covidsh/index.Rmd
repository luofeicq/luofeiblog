...
date_sh <- pages %>% 
  html_node(".time") %>% 
  html_text()
data.frame( Date = date_sh,  Title = title_sh, Url = url2_sh)
}
...
url_2 <-  pages_1 %>% dplyr::filter(str_detect(Title, "居住地信息")) %>% mutate(
  url_2 = paste0("https://wsjkw.sh.gov.cn/", Url) ) %>% select(url_2) %>% unlist()


webfun2 <- function(url){
test <-  read_html(url) %>% html_nodes("p") %>% html_text() %>% tibble()
colnames(test) <- "Text"
test %>% 
  dplyr::filter(str_detect(Text, "分别居住")) %>% 
  separate(Text, c("Date", "col1", "col2"), sep = "，|、") %>% 
  mutate(
    Date_rep = as.Date.character(Date, "%Y年%m月%d日"),
    Local = str_extract(col1, "..."), 
    case_conf = str_extract(col1, "[0-9]+") %>% as.numeric(),
    case_none = str_extract(col2, "[0-9]+") %>% as.numeric()
  ) %>% select(Date_rep, Local, case_conf, case_none)
}

final_df <- map_df(url_2, webfun2)
final_df <- final_df %>%  dplyr::filter(!is.na(Date_rep))  
...
write.xlsx(final_df, "data/shcovid.xlsx")
```

```{r, warning=FALSE, include=TRUE, results='asis' }
final_df <- read_excel("data/shcovid.xlsx")
```
...
final_df %>% ggplot(aes(x = factor(Date_rep), y = case_sum)) +
  ...
  ylim(c(-300,13000)) + geom_text(data = final_df %>% dplyr::filter(Local == "浦东新区"), aes(label = Date_rep ),
                                  nudge_y = 3000,
                                  size = 2, 
                                  ) +
  guides(fill = guide_legend(ncol = 2)) +
  theme(plot.margin = unit(rep(0.1,4), "pt"))
```
