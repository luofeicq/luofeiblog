...
首先看看随着时间的大体趋势，感染者数量几乎是成指数增加，仿佛仍然没到头。但是从风险人群中检出感染者数量似乎有下降的迹象。
```{r}
...
再看看每天感染者发现途径的变化情况，红色代表每天从风险人群中发现感染者，蓝色代表从管控人群。可预见从风险人群中发现感染者的比例大体趋势是减少的。这种情况视乎是一个好的征兆。但是是否存在随着时间变化，防控政策的变化，同时对两类人群的定义是否也随之变化？
```{r}
tb_fenlei_long %>% ggplot(aes(x = rep_date, y = cases)) +
  geom_col(aes(fill = type), position = "fill") +
  scale_x_date(date_breaks = "2 day", date_labels = "%m-%d", expand = c(0.05,0.05))
```
再单独看看风险人群中检出感染者的情况，确实出现了4月3日，4月4日连续两天下降，且看起来几乎是呈直线下降，这似乎是一个好兆头，我们再观察几天看。
...
tb_fenlei_long %>% filter(type == "case_all_fx") %>% 
  ...
  geom_text(data = .%>% filter(rep_date > "2022-04-01"),aes(label = cases), nudge_y = 50, nudge_x = 1 , col = 4)
```
