...
newdata <- newdata %>% mutate(
  pred = predict(mod1, newdata) %>% round(0)
)

mod_data %>% ggplot() +
  geom_line(aes(days, case_all), col = 2)+
  geom_line(data = newdata, aes(days, pred), col = 3) +
  geom_point(data = newdata, aes(days, pred), col = 3) +
  geom_text(data = newdata %>% filter(days > 22), aes(days, pred, label =  pred)) +
  theme_bw()
